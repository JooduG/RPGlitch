...
const checks = [
  {
    name: "node",
    cmd: process.platform === "win32" ? "where node" : "which node",
  },
  { name: "npx", exists: !!which("npx") },
  { name: "uvx", exists: !!which("uvx") },
  {
    name: "python",
    cmd: process.platform === "win32" ? "where python" : "which python",
  },
...
for (const c of checks) {
  if (typeof c.exists === "boolean") {
    console.log(
      `${c.name.padEnd(20)} ${c.exists ? "OK" : "NOT FOUND in PATH"}`,
    );
  } else if (c.env) {
    console.log(`${c.name.padEnd(20)} ${c.env}`);
  } else if (c.cmd) {
    const res = spawnSync(c.cmd.split(" ")[0], c.cmd.split(" ").slice(1), {
      encoding: "utf8",
    });
    console.log(
      `${c.name.padEnd(20)} ${res.status === 0 ? "OK" : "NOT FOUND"}`,
    );
  }
}
console.log(
