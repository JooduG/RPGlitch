  ...
  _onData(chunk) {
    ...
    while (true) {
      ...
      try {
        ...
        this.queue.push(msg);
      } catch (e) { void e; /* ignore malformed JSON messages */ }
    }
  }
  send(msg) {
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === '--only' && argv[i + 1]) {
      ...
      args.skip = new Set(argv[++i].split(',').map((s) => s.trim()).filter(Boolean));
...
async function testServer(name, srv, allEnv, opts) {
  ...
  if (srv.env && typeof srv.env === 'object') {
    for (const [k, v] of Object.entries(srv.env)) {
      const replaced = String(v).replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g, (_, key) => allEnv[key] ?? '');
      env[k] = replaced;
    }
  }
  ...
  proc.on('error', (e) => { stderrBuf = Buffer.concat([stderrBuf, Buffer.from(String(e?.message || e))]); });
  proc.stdout.on('data', (d) => { stdoutBuf = Buffer.concat([stdoutBuf, d]); });
  proc.stderr.on('data', (d) => { stderrBuf = Buffer.concat([stderrBuf, d]); });
  proc.on('exit', (code, signal) => { exitInfo = { code, signal }; });

  // Hard timeout watchdog
  const watchdog = setTimeout(() => {
    timedOut = true;
    try { proc.kill('SIGTERM'); } catch (e) { void e; }
  }, opts.maxMs || 20000);
  ...
  const initResp = await rpc.waitFor((m) => m.id === 1 && (m.result || m.error), 8000);
  ...
  const init = initResp || await rpc.waitFor((m) => m.id === 101 && (m.result || m.error), 6000);
  ...
  let toolsResp = await rpc.waitFor((m) => m.id === 2 && (m.result || m.error), 6000);
  if (!toolsResp) {
    rpc.send({ jsonrpc: '2.0', id: 13, method: 'tools.list' });
    toolsResp = await rpc.waitFor((m) => m.id === 13 && (m.result || m.error), 6000);
  }
  if (toolsResp && toolsResp.result && Array.isArray(toolsResp.result.tools)) {
    ...
    if (names.includes('list_dir')) {
      rpc.send({ jsonrpc: '2.0', id: 3, method: 'tools/call', params: { name: 'list_dir', arguments: { path: '.' } } });
      const callResp = await rpc.waitFor((m) => m.id === 3 && (m.result || m.error), 6000);
      toolCallOk = !!(callResp && callResp.result);
    }
  }

  // wrap up
  setTimeout(() => { try { proc.kill('SIGTERM'); } catch (e) { void e; } }, 500);
  ...
  const meta = { name, cmd, args, initializeOk, toolsListOk, toolsCount, toolCallOk, exitInfo, stdoutBytes: stdoutBuf.length, stderrBytes: stderrBuf.length };
  fs.writeFileSync(metaPath, JSON.stringify(meta, null, 2));

  const status = timedOut ? 'timeout' : (initializeOk ? 'ok' : (started ? 'started' : 'no-output'));
  return { name, type: 'stdio', status, toolsListOk, toolsCount, toolCallOk, stdout: stdoutPath, stderr: stderrPath, meta: metaPath };
}
...
async function run() {
  ...
  fs.writeFileSync(summaryPath, JSON.stringify({ when: new Date().toISOString(), results }, null, 2));

  console.log('MCP Test Summary');
  console.log('='.repeat(72));
  for (const r of results) {
    const parts = [r.name.padEnd(28), String(r.type).padEnd(6), r.status.padEnd(10)];
