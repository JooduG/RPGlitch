...
export function getPictureHTML(entity, options = {}) {
  ...
  if (url) {
    ...
    // [POLLINATIONS] Use distinct icons for Fractals vs Characters
    const isFractal = (entity.type || "").toLowerCase() === "fractal";
    const iconPath = isFractal
      ? "M12 2L2 12l10 10 10-10L12 2zm0 3.5L18.5 12 12 18.5 5.5 12 12 5.5z" // Diamond
      : "M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"; // User

    div.innerHTML = `<svg class="placeholder-icon" viewBox="0 0 24 24" fill="currentColor" style="width:33%; height:33%; opacity:1;"><path d="${iconPath}"/></svg>`;
  }
