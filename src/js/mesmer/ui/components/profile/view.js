...
export const renderProfileView = async (
  ...
  let localVisuals = VisualManager.getVisualState(entity);
  ...
  flipBtn.onclick = async (e) => {
    ...
    if (id !== "new") {
      try {
        ...
        const { events, EVENTS } =
          await import("../../../../gamemaster/index.js");
  ...
  const createRow = (groupKey, groupConfig) => {
    ...
    if (groupConfig.type === "nested") {
      ...
      keys.forEach((key) => {
        ...
        // Only show headers for "eternal" to avoid duplicates in "present"
        if (groupKey === "eternal") {
          const header = document.createElement("div");
