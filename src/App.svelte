...
<main class="app-root">
    {#if app.view === 'lobby'}
         ...
         <div class="hud-layer"><div class="bottom-right"><SettingsButton /></div></div>
    {:else}
        <Stage transparent={true} />
        <div class="hud-layer">
    ...
    {#if app.controlPanelOpen} <ControlPanel /> {/if}
    {#if app.profileOpen} <Profile /> {/if}
</main>

<style lang="scss">
    @import './artificer/base.scss';
    .app-root { position: relative; width: 100vw; height: 100vh; overflow: hidden; pointer-events: none; }
    .hud-layer { position: absolute; inset: 0; pointer-events: none; z-index: 50; padding: 1rem; 
        .top-right { position: absolute; top: 1rem; right: 1rem; pointer-events: auto; }
        .bottom-right { position: absolute; bottom: 1rem; right: 1rem; pointer-events: auto; }
    }
</style>
