...
{#if meta.type === "MEMORY_FORMATION"}
  ...
  <div class="flex flex-col gap-3">
    ...
    {#if meta.future}
      <div
        class="flex flex-col gap-1 rounded-sm border border-[color-mix(in_srgb,var(--color-dev-accent),transparent_80%)] bg-[color-mix(in_srgb,var(--color-dev-accent),transparent_95%)] p-2.5 text-xs"
      >
    ...
    {#if meta.present?.non_physical || meta.present?.physical}
      <div class="flex flex-col gap-1 rounded-sm border border-slate-800 bg-slate-900/40 p-2.5 text-xs">
      ...
      <div class="flex flex-col gap-2 pt-1">
        ...
        <div class="flex flex-col gap-2">
          {#each forged_vectors as v, i (v.id || v.content)}
            <div
              class="flex animate-[slide-in_300ms_cubic-bezier(0.4,0,0.2,1)_both] gap-3 rounded-sm border border-l-4 border-[color-mix(in_srgb,var(--color-dev-accent),transparent_85%)] border-l-(--color-dev-accent) bg-[color-mix(in_srgb,var(--color-dev-accent),transparent_95%)] px-2.5 py-2 text-xs leading-relaxed"
