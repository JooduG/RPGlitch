/**
 * src/ui/message/telemetry.js
 * 📊 TELEMETRY DISPLAY HELPERS
 * Pure normalization/formatting for the telemetry feed (TelemetryCard,
 * TelemetryBlocks, TelemetryVector). Converts the `{ type, updates }` event
 * shape into the blocks the feed renders, resolves entity keys to display
 * names, and formats memory/vector type labels. Rendered by the message UI
 * only — the kernel builds the raw payload via @intelligence/telemetry.js.
 */
