<script>
  ...
  import { pulse, roll, shimmy } from "@motion";
...
<GlassPill class="interactable pill-chassis">
  {#snippet left()}
    <Button
      class="flank"
      variant="invisible"
...
<style>
  /* Structural side panel layouts and dimensions are now managed automatically by GlassPill */

  :global(.action.is-ready h6) {
    color: var(--emerald-green);
    text-shadow: 0 0 calc(var(--spacing-unit) * 2)
      rgb(from var(--emerald-green) r g b / var(--opacity-whisper));
  }
