 ...
 * Canonical registry of supported speaking styles and vocabulary substitution dictionaries.
 */

// ── 1. Declarative Rule Builder ───────────────────────────────────────────────
...
export const COMMUNITY_RULES = [
  ...
  rule(/([^A-Za-z])void\s+of\b/gi, (match, p1, ...args) => {
    const offset = args[args.length - 2];
    const list = ["empty of", "bare of", "lacking"];
    return p1 + list[Math.abs(offset) % list.length];
  }),
...
// ── 7. Consolidated Speaking Style Rules Registry ─────────────────────────────
...
/**
 * Validates whether a given string is a supported speaking style.
 * @param {any} val
 * @returns {boolean}
 */
export function is_valid_speaking_style(val) {
  return typeof val === "string" && VALID_SPEAKING_STYLES.has(val);
}
