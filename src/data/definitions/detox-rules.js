 ...
 * Extracted from normalizer.js so the entity-normalization module stays lean.
 *
...
export function detox_prose(rawText, register = "plain") {
  ...
  const DETOX_RULES = [
    ...
    // 11. REDDIT-DERIVED AI-ISMS (community-reported fetish words)
    // Sourced from r/perchance complaints. These are mechanical, unambiguous word/phase
    // swaps. Anything too context-dependent for a scrubber (silt, scar, tap, anchor,
    // sanctuary, holograph, em-dashes, recycled fantasy names, the "X didn't just Y; it
    // Z'd" construction) is handled by the PROSE_STRUCTURE protocol instead of here.
    {
    ...
    },
    {
      ...
      replace: (match, ...args) => {
        const noun = args[0];
        const offset = args[args.length - 2];
        return stable_pick(["golden", "honeyed", "warm gold", "soft gold"], match, offset) + " " + noun;
      },
    },
    {
    ...
    },
    {
      ...
      replace: (match, ...args) => {
        ...
        const key = p1 === "ing" ? "ing" : "ed";
        const active = forms[exact_voice] || forms[fallback_voice] || forms.plain;
        return match_case(match, stable_pick(active[key], match, offset));
      },
    },
    {
    ...
    },
    {
      ...
      replace: (match, ...args) => match_case(match, args[0] + "'d been holding"),
    },
    {
