...
import arcanaPrompts from "@data/library/tarot.json"
...
const REFLEX_REGISTRY = [
    {
        id: "KINETIC",
        regex: /kill(ed|ing)?|shoot|shot|stabb?(ed|ing)?|punch(ed|ing)?|hit|fought|fight(ing)?|attack(ed|ing)?|gunn?(ed|ing)?|weapon|blood|hurt|destroy(ed|ing)?|br(eak|oke)n?|smash(ed|ing)?|burn(t|ed|ing)?|explosion|die(d|ing)?|fast|speed|impact|crash(ed|ing)?|loud|bang|boom|slam(med|ming)?|athletics?|run(ning)?|jump(ed|ing)?/i,
        deltas: () => ({
            velocity: PHYSICS_CONSTANTS.REFLEX_VIOLENCE_VELOCITY,
        }),
    },
    {
        id: "VULNERABLE",
        regex: /kiss(ed|ing)?|lov(e|ed|ing)|h(e|o)ld(ing)?|hugg?(ed|ing)?|touch(ed|ing)?|gentle|soft|warm|caress(ed|ing)?|cheek|hand|finger|whisper(ed|ing)?|close|safe|trust(ed|ing)?|heart|blush(ed|ing)?|honesty?|truth|tear|cry(ing)?|sad|lonely|ache|naked|bare|skin/i,
        deltas: () => ({
    ...
    {
        id: "UNSTABLE",
        regex: /scream(ed|ing)?|r(un|an)(ning)?|hid(e|den|ing)?|fear(ed|ing)?|scar(ed|ing)?|dark|shadow|weird|glitch(ed|ing)?|wrong|monster|ghost|dead|rot(ted|ting)?|decay(ed|ing)?|cold|shiver(ed|ing)?|nightmare|static|noise|distortion|fracture(d|ing)?|chaos|erratic|flicker(ed|ing)?/i,
        deltas: () => ({ entropy: PHYSICS_CONSTANTS.REFLEX_FEAR_ENTROPY }),
    },
    {
        id: "HARMONIC",
        regex: /underst(ood|and)(ing)?|connect(ed|ing)?|sync(hroniz)?(ed|ing)?|resonance|empathy|harmony|together|shared|mind|soul|link(ed|ing)?|bond(ed|ing)?|mirror(ed|ing)?|echo(ed|ing)?|melody|music|rhythm|vibe|frequency|pulse/i,
        deltas: () => ({
            resonance: PHYSICS_CONSTANTS.REFLEX_SYNCHRONY_RESONANCE,
        }),
    },
    {
        id: "STASIS",
        regex: /br(eathe|eath)(d|ing)?|wait(ed|ing)?|pause(d|ing)?|stop(ped|ping)?|still|silence|quiet|calm(ed|ing)?|rest(ed|ing)?|sleep|slow(ed|ing)?|wait/i,
        deltas: () => ({ velocity: PHYSICS_CONSTANTS.REFLEX_STASIS_VELOCITY }),
    },
    {
        id: "SHIELDED",
        regex: /armor(ed)?|shield(ed|ing)?|wall|mask(ed|ing)?|guard(ed|ing)?|protect(ed|ing)?|block(ed|ing)?|shell|barrier|hide|hid(den)?/i,
        deltas: () => ({
    ...
    {
        id: "ORDERED",
        regex: /logic(al)?|math(ematics)?|calculat(e|ed|ing)|precise|clean(ed|ing)?|pure|order(ed|ing)?|structur(e|ed|ing)|fact|prove(n|d)?|proof/i,
        deltas: () => ({ entropy: PHYSICS_CONSTANTS.REFLEX_ORDERED_ENTROPY }),
    },
    {
        id: "DETACHED",
        regex: /data|clinical|observ(e|ed|ing)|watch(ed|ing)?|analy[sz](e|ed|ing)|study(ing)?|scan(ned|ning)?|test(ed|ing)?|monitor(ed|ing)?|variable/i,
        deltas: () => ({
            resonance: PHYSICS_CONSTANTS.REFLEX_DETACHED_RESONANCE,
        }),
    },
]
