...
import { physics_engine, evaluate_physics_signals, DYNAMICS_META } from "./physics.js";
...
describe("physics.js", () => {
  ...
  describe("physics_engine._get_baselines", () => {
    it("returns dynamics_baseline when set", () => {
      const entity = { dynamics_baseline: { chaos: 42, intensity: 55 } };
      const baselines = physics_engine._get_baselines(entity);
    ...
    it("returns empty object when entity is null or undefined", () => {
      expect(physics_engine._get_baselines(null)).toEqual({});
      expect(physics_engine._get_baselines(undefined)).toEqual({});
    });
  });

  describe("physics_engine.settle_physics", () => {
    it("pulls dynamics toward baselines", () => {
      ...
      for (let i = 0; i < 50; i++) {
        physics_engine.settle_physics(dynamics, baselines, 50, 0.15);
      }
    ...
    it("clamps values to 0-100 bounds", () => {
      ...
      physics_engine.settle_physics(dynamics, {}, 50, 0.1);
    ...
    it("does nothing when dynamics is null or not an object", () => {
      expect(() => physics_engine.settle_physics(null, {}, 50, 0.1)).not.toThrow();
      expect(() => physics_engine.settle_physics(undefined, {}, 50, 0.1)).not.toThrow();
    });
  });

  describe("evaluate_physics_signals (Unified Signals & Triggers Engine)", () => {
    it("evaluates global baseline triggers when intensity or entropy cross thresholds", () => {
      ...
      const signals = evaluate_physics_signals({ ai_dynamics, fractal_dynamics });
    ...
    it("combines global triggers and active style triggers", () => {
      ...
      const signals = evaluate_physics_signals({ ai_dynamics, style });
      const ids = signals.map((s) => s.id);
    ...
    it("evaluates composite triggers for dual-axis conditions", () => {
      ...
      let signals = evaluate_physics_signals({ ai_dynamics: { openness: 20, affinity: 20 } });
      let ids = signals.map((s) => s.id);
      expect(ids).toContain("SUSPICION");

      // CATACLYSM (velocity > 70 && entropy > 70)
      signals = evaluate_physics_signals({ fractal_dynamics: { velocity: 80, entropy: 80 } });
      ids = signals.map((s) => s.id);
      expect(ids).toContain("CATACLYSM");

      // CONFESSION (openness > 70 && velocity < 30)
      signals = evaluate_physics_signals({ ai_dynamics: { openness: 75 }, fractal_dynamics: { velocity: 25 } });
      ids = signals.map((s) => s.id);
      expect(ids).toContain("CONFESSION");

      // PASSION (intensity > 70 && affinity > 70)
      signals = evaluate_physics_signals({ ai_dynamics: { intensity: 80, affinity: 80 } });
      ids = signals.map((s) => s.id);
      expect(ids).toContain("PASSION");

      // TRANCE (intensity < 30 && chaos > 70)
      signals = evaluate_physics_signals({ ai_dynamics: { intensity: 20, chaos: 85 } });
      ids = signals.map((s) => s.id);
      expect(ids).toContain("TRANCE");

      // HARMONY (chaos < 30 && entropy < 30) suppresses single-axis STABILITY & RECOVERY
      signals = evaluate_physics_signals({ ai_dynamics: { chaos: 20 }, fractal_dynamics: { entropy: 20 } });
      ids = signals.map((s) => s.id);
      ...
      signals = evaluate_physics_signals({ ai_dynamics: { chaos: 50 }, fractal_dynamics: { entropy: 20 } });
      ids = signals.map((s) => s.id);
    ...
    it("emits zero signals when dynamics remain neutral", () => {
      ...
      const signals = evaluate_physics_signals({ ai_dynamics, fractal_dynamics, style: NARRATIVE_STYLES.default });
