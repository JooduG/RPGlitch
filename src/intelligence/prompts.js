...
<SYSTEM role="${escapeXml(entities.AI.name)}" round="${escapeXml(String(round))}">
  ...
  <TASK>
    ${stabilityLockContent ? `${tag_block("STABILITY_LOCK", stabilityLockContent, 4)}\n` : ""}
...
<SYSTEM role="${escapeXml(entities.FRACTAL.name)}"${round != null ? ` round="${escapeXml(String(round))}"` : ""} mode="${mode.toUpperCase()}">
  ...
  ${tag_block("PROTOCOLS", prompt_builder.render_protocols("COGNITION, PRESENT_TENSE, HYGIENE, USER_AGENCY, MOMENTUM, MARKDOWN_FORMAT"), 2)}
  ${tag_block("NARRATIVE_STYLE", authorStyleContent, 2, authorAttr)}
  <TASK>
    <IDENTITY>
...
${tag_block("PROTOCOLS", prompt_builder.render_protocols("HYGIENE, AFFIRMATIVE, PRESENT_TENSE"), 0)}
<CONTEXT>
...
Declare the finalized emotional state vectors and immediate intent.`,
  ...
  MARKDOWN_FORMAT:
    "You MUST use markdown formatting to partition prose cleanly. Use *italics* exclusively for internal character reflections, observations, or unspoken psychological transitions. Do NOT use bold typography for physical actions or narrative sound effects; reserve bold formatting strictly for major structural concepts or explicit timeline shifts.",
  JSON_OUTPUT:
