...
-   **Atomic CSS:** All styling for **global utilities and shared layout patterns** must use atomic (utility) CSS classes—one class, one property. No element selectors, no deep nesting, and no `!important` for these global styles. All core utilities are defined in `atomic-utilities.css`. If a needed global utility does not exist, add it there.
-   **Component Styling:**
    ...
    -   Component styling should complement, not override, the core atomic utilities.
...
-   **Card Components:**
    ...
    -   **Text Overflow:** Smart ellipsis handling with `text-wrap: balance` and fallback overflow styles
    -   **Color Integration:** "Premade" tags use character's color palette for background colors
    -   **Overflow Protection:** All card info areas use `overflow: hidden` to prevent background "leakage" and ensure clean visual boundaries
    -   **CSS Specificity:** Card-specific styles use `:not()` selectors to exclude cards from general article rules, preventing conflicts
...
-   [ ] All cards use semantic HTML structure with `<article>`, `<header>`, `<main>`, `<footer>` elements.
-   [ ] Card info areas have `overflow: hidden` to prevent background "leakage".
-   [ ] Text overflow is handled with `text-wrap: balance` and ellipsis fallbacks.
-   [ ] "Premade" tags use character's color palette for background colors.
-   [ ] Card-specific styles use `:not()` selectors to exclude cards from general article rules.
-   [ ] All cards maintain consistent padding, spacing, and typography.
-   [ ] Cards are responsive and touch-friendly.
-   [ ] No background leakage or visual artifacts around card edges.
