...
class AtomicClassGenerator {
  ...
  extractDesignTokens() {
    ...
    const spacingMatches =
      this.cssContent.match(/--([a-z-]+)-spacing:\s*([^;]+);/g) || [];
    ...
    const fontSizeMatches =
      this.cssContent.match(/--font-size-[a-z0-9-]+:\s*([^;]+);/g) || [];
    ...
    const colorMatches =
      this.cssContent.match(/--([a-z-]+)-color:\s*([^;]+);/g) || [];
    ...
    const borderRadiusMatches =
      this.cssContent.match(/--([a-z-]+)-border-radius:\s*([^;]+);/g) || [];
  ...
  generateAtomicClasses() {
    let atomicClasses =
      "/* Dynamically Generated Atomic Utility Classes */\n\n";

    // Spacing utilities
    atomicClasses += "/* Margin Utilities */\n";
    [0, 0.25, 0.5, 1, 1.5, 2].forEach((size) => {
      atomicClasses += `.m-${size
        .toString()
        .replace(".", "-")} { margin: ${size}rem; }\n`;
      atomicClasses += `.mx-${size
        .toString()
        .replace(
          ".",
          "-"
        )} { margin-left: ${size}rem; margin-right: ${size}rem; }\n`;
      atomicClasses += `.my-${size
        .toString()
        .replace(
          ".",
          "-"
        )} { margin-top: ${size}rem; margin-bottom: ${size}rem; }\n`;
    });

    // Padding utilities
    atomicClasses += "\n/* Padding Utilities */\n";
    [0, 0.25, 0.5, 1, 1.5, 2].forEach((size) => {
      atomicClasses += `.p-${size
        .toString()
        .replace(".", "-")} { padding: ${size}rem; }\n`;
      atomicClasses += `.px-${size
        .toString()
        .replace(
          ".",
          "-"
        )} { padding-left: ${size}rem; padding-right: ${size}rem; }\n`;
      atomicClasses += `.py-${size
        .toString()
        .replace(
          ".",
          "-"
        )} { padding-top: ${size}rem; padding-bottom: ${size}rem; }\n`;
    });
    ...
    [0, 0.25, 0.5, 1].forEach((size) => {
      atomicClasses += `.rounded-${size
        .toString()
        .replace(".", "-")} { border-radius: ${size}rem; }\n`;
    });
