...
function readJson(filePath) {
    if (!fs.existsSync(filePath)) {
        console.warn(`⚠️  File not found: ${path.relative(REPO_ROOT, filePath)}. Returning empty object.`);
        return {}; // Return empty object instead of null
    }
    try {
        let content = fs.readFileSync(filePath, 'utf8');
        if (content.trim() === '') {
             console.warn(`⚠️  File is empty: ${path.relative(REPO_ROOT, filePath)}. Returning empty object.`);
             return {}; // Handle empty file
        }
        if (content.charCodeAt(0) === 0xFEFF) content = content.slice(1);
        return JSON.parse(content);
    } catch (e) {
        console.error(`❌  Could not parse JSON from ${path.relative(REPO_ROOT, filePath)}. Returning empty object.`, e);
...
async function syncLibs() {
    ...
    const getFileChecksum = (filePath) => crypto.createHash('sha256').update(fs.readFileSync(filePath)).digest('hex');

    const download = (url, dest, sha256) => new Promise((resolve, reject) => {
    ...
    await Promise.all(SOURCES.map(async ({ url, file, sha256 }) => {
...
function syncIgnores() {
    ...
    jsconfig.exclude = Array.from(new Set(['node_modules', ...getPatterns('gitignore'), ...getPatterns('ide')])).sort();
    writeJson(jsconfigPath, jsconfig);

    const settings = readJson(settingsPath); // No || {} needed
    settings['files.exclude'] = { ...(settings['files.exclude'] || {}), ...(masterIgnores.vscode?.filesExclude || {}) };
...
function syncMcp() {
    ...
    const envMap = fs.existsSync(envPath) ? { ...process.env, ...dotenv.parse(fs.readFileSync(envPath)) } : { ...process.env };

    const substituteEnvVariables = (obj) => JSON.parse(
        JSON.stringify(obj).replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g, (match, key) => envMap[key] || match)
    );

    const resolvedMcp = substituteEnvVariables(masterMcp);

    writeJson(path.join(REPO_ROOT, 'mcp.json'), resolvedMcp);
    writeJson(path.join(REPO_ROOT, '.vscode', 'mcp.json'), resolvedMcp);
...
function generateHub() {
    ...
    const getRepoTree = () => {
        const exclude = [
            /node_modules/, /\.git/, /\.vscode/, /build\/local_libs/, /^\.yarn/
        ];
        const output = [];
    ...
    const getPackageScripts = () => {
        try {
            ...
            return important
                ...
                .join('\n');
        } catch { return 'Could not load scripts.'; }
    };
...
async function main() {
    ...
    if (runAll || args.configs) syncConfigs();
