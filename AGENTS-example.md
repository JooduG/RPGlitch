...
- VERIFY your changes with `browser_eval`. It runs JS inside the live page (rendered by the real perchance engine, visible to the user). If you edited files since your last eval, the live page is automatically updated and hard-reloaded first.
...
- Read generator state: `return root.character.hp` — everything top-level in main.pjs is on `root` (and on `window` outside module scripts).
...
- Console/errors aren't captured automatically; if you suspect an error, check with e.g. `return typeof someFn` or wrap logic in try/catch inside your eval.
...
- `browser_refresh` — hard-reload the live output page with your current code and wait for readiness. Use it if browser_eval reports the page frozen, or when you want a clean slate. Its result includes the fresh page's console output and any `perchanceErrors` the engine reported — so ALWAYS finish a task that changed code with a final `browser_refresh` (or a verifying `browser_eval`) and confirm the page is error-free before reporting done. Optional `preambleJs`: JS that runs in the fresh page BEFORE any generator code (before pjs rendering and body scripts) — the only way to observe load-time behavior, e.g. wrap `window.fetch`/`WebSocket` to log calls. Scoped to that one page load, like anything you set up via browser_eval: the next reload starts clean, so pass it again if you still need it.
- browser_eval/browser_refresh results also include `syntaxErrors` whenever your current main.pjs/index.html contain a JS syntax error, with REAL file line numbers. A runtime `SyntaxError`'s line number does NOT map to your files (scripts are re-injected with wrappers) — when you see one, fix the location `syntaxErrors` points at instead.

Your edits ARE reliably applied: a browser_eval never runs against stale code — if you edited anything since the last push, the page is hard-reloaded with your current code first (verified/guaranteed). If you did NOT edit anything, the page is deliberately NOT reloaded, so page state (variables, DOM changes, wraps from earlier evals) persists between evals; pass `reload: true` if you want a fresh page anyway. So if an element or value is missing, it's your code or a TIMING issue, never the tooling. Two common timing pitfalls:
...
Only import what you actually use. After adding an import, your next `browser_eval` runs against a reloaded page with the plugin available — you can test it like, e.g. `return await root.kv.folderName.entries()`.
...
IMPORTANT: always access imported plugins through `root` (e.g. `root.generateText(...)`, `root.kv.folder.get(...)`) in `browser_eval`, scripts, modules, inline handlers, and pjs square-bracket blocks. Keep only the top-level import assignment itself bare (`generateText = {import:ai-text-plugin}`).
...
await root.generateText({
  ...
  onChunk: (data) => {
    storyEl.textContent += data.textChunk;
  },
});
```

The returned Promise has a special feature: assigned to an element's `innerHTML`, it streams in automatically (`outputEl.innerHTML = root.generateText("...")`), and it has a `.stop()` method. Generation can take up to a minute — ALWAYS show a loading indicator in the UI's that you build.
...
See the `ai-text-plugin` skill for tips on designing prefix-cache-friendly prompt templates which result in much faster generation.
...
Takes a few seconds — show a loading indicator. Prefer large resolution for higher quality, and scale down with CSS if needed. For static placeholder images, prefer an inline SVG.
...
### superFetch (super-fetch-plugin) — fetch without CORS problems
...
await root.kv.myFolder.setMany([
  ["a", 1],
  ["b", 2],
]);
...
- Keep everything in the two files; no external stylesheets/scripts except CDN imports (prefer `esm.sh` for modules - e.g. `import * as THREE from "https://esm.sh/three@<version>"`). Use vanilla JS/HTML/CSS unless a library genuinely helps.
- Use the `hidden` attribute to hide elements (`myEl.hidden = true`) instead of display:none — in the Perchance engine the `hidden` attribute _always_ wins over inline styles, so you can use it confidently.
...
- Always show a loading indicator for async operations (generateText/generateImage can take up to a minute).
...
- Unless unreasonable or instructed otherwise, applications should generally responsively adapt to screen resolution / aspect ratio.
...
- This is a emphemeral chat session. If the user refreshes the browser tab, a new/fresh chat session may be created — with a fresh workspace: only main.pjs + index.html carry over (see Workspace).
...
- If the application requires WebGPU/WebGL, then it's a good idea to create a `$meta.image` (screenshot or AI-generated image, or a combination) that represents the application, since the platform-level screenshotter may not support WebGPU/WebGL, so it won't look nice in the generator listings page, which shows a screenshot of the generator if it doesn't have a `$meta.image`.
  encoding:
  utf8
  mime:
  text/markdown
