...
| 2026-09-06 02:08 | Intelligence Prompt Builder Modernization: (1) Deduplicated scoring context assembly in `create_render_accessors` by delegating directly to `build_scoring_context`; (2) Standardized nomenclature per GEMINI.md Anti-Abbreviation Mandate (`entry`, `index`, `turn_number`, `message`, `reference`); (3) Harmonized `resolve_accessors` to symmetrically inspect `payload.raw_messages || payload.simulation_log`; (4) Enforced P4 deterministic `build_prologue` without silent character fallbacks; (5) Standardized window bridge to `window.exposed.prompt_builder`; (6) Verified 100% clean passes: 7/7 tests in `builder.test.js`, 62/62 tests in `story-pipeline.test.js`, and 14/14 hook contracts. | `implement`, `test` | ✅ Completed |
