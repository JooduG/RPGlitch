...
- [ ] `track-4-ui-ux-harmonization-2026-08-24`: UI/UX Harmonization, Entity Databoxes, Choreography & Interactive Guards ([tracks/track-4-ui-ux-harmonization.md](tracks/track-4-ui-ux-harmonization.md))

### Recent Completed Tracks

- [x] `track-4-ui-ux-harmonization-2026-08-24`: UI/UX Harmonization, Single-Entity Telemetry Databox, Message Header Pin & Story Navigation ([tracks/track-4-ui-ux-harmonization.md](tracks/track-4-ui-ux-harmonization.md)) — Single-entity forged databox in pure terminal cyan (`var(--color-dev-accent)`) with dynamics deltas and relational text vectors, click-to-pin message header & action toolbar interaction model via `app.pinned_message_id`, dynamic `"ENTER STORYMODE"` bottom bar resume button on Storyboard, and interactive failed image cards with `[Retry]` and `[Dismiss]`.
- [x] `track-3-decoupled-image-cooldowns-2026-08-24`: Decoupled Image Trigger Cooldowns, Round Ceiling, Cinematic Framing & Image Card Harmonization ([tracks/track-3-decoupled-image-cooldowns.md](tracks/track-3-decoupled-image-cooldowns.md)) — Decoupled independent cooldown timers (`director_cooldown_rounds = 2`, `dynamics_cooldown_rounds = 3`), per-source timestamps (`last_director_beat_round`, `last_dynamics_beat_round`), Priority 1 (Director) > Priority 2 (Dynamics) arbitration with a strict 1-image-per-round ceiling, dynamic photographic framing lenses (`Intimate Close-Up`, `Medium Action`, `Wide Environmental`, `Dutch / Low-Angle`) in `src/media/image-prompts.js`, and harmonized card chassis styling.
- [x] `track-2-back-shot-rolling-worker-2026-08-24`: Back Shot Rolling Worker, Single-Entity State & Relational Mesh ([tracks/track-2-back-shot.md](tracks/track-2-back-shot.md)) — Single-entity Back Shot consolidation (`BACK_SHOT_JSON_SCHEMA`), per-entity unconsolidated message progress markers (`meta.forged_entities`), round-robin cursor rotation with auto-advancement over inactive entities, outbound relational mesh mutations dispatched to `kernel._apply_relationships`, and legacy promotion schema/method purge.
- [x] `track-1-director-quick-shot-2026-08-24`: Director Quick Shot, Unified Next Action & Latency Instrumentation ([tracks/track-1-quick-shot.md](tracks/track-1-quick-shot.md)) — High-resolution Director execution timer (`performance.now()`), `last_director_ms` / `director_ms_pool` rolling ring buffer, `generation_mutex` state in `runtime.svelte.js`, streamlined 4-field Quick Shot schema (`next_action`, `keywords`, `directors_note`, `dynamics_deltas`), and inline synchronous Genesis synthesis.
