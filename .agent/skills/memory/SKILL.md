...
1. **Pause & Reflect**: Before planning a new mechanic or touching core logic, explicitly identify what you *don't* know about the codebase's history or previous implementations.
...
1. **Search**: Use the `read_knowledge_base` tool to query the Pinecone vector database for highly specific historical context.
    - *Note:* The database is divided into three strict namespaces: `knowledge-base.meta` (rules), `knowledge-base.external` (libraries), and `knowledge-base.src` (source code).
...
*This phase triggers at the end of a session, right before clocking out.*
