...
/**
 * @file .agent/skills/SWARM/scripts/swarm.js
 * 
 * ─────────────────────────────────────────────────────────────────────────────
...
import { llm_service } from "../../../../src/core/intelligence/llm-service.js";
import { SWARM_TEMPLATES } from "../templates/swarm-templates.js";

export class SwarmEngine {
  // --- PRIVATE STATE (#) ---
  #tasks = $state([]);
  #processed_count = $state(0);
  #active_agent_count = $state(0);
  #errors = $state([]);

  // --- PUBLIC GETTERS (REACTIVE) ---
  get tasks() { return this.#tasks; }
  get processed_count() { return this.#processed_count; }
  get active_agent_count() { return this.#active_agent_count; }
  ...
  async dispatch_manifest(manifest, options = { max_concurrency: 3, stop_on_error: false }) {
    ...
    return {
      ...
      results: $state.snapshot(this.#tasks),
    };
  }
  ...
  static async bootstrap(args) {
    ...
    switch (command) {
      case "review": await engine.cli_review(args, flags); break;
      case "brief": this.show_meta(flags); break;
