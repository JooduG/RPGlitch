...
4. **Tracing**: Use `knowledge/root-cause-tracing.md` to work backward from the symptom to the source.
...
3. **Defense in Depth**: Once the root cause is fixed, add safeguards (see `knowledge/defense-in-depth.md`) to prevent recurrence.
...
- **[Root Cause Tracing](./knowledge/root-cause-tracing.md)**: Deep dive into tracing mechanics.
- **[Defense in Depth](./knowledge/defense-in-depth.md)**: How to harden code after a fix.
- **[Condition Based Waiting](./knowledge/condition-based-waiting.md)**: Solving "flaky" timing bugs.
