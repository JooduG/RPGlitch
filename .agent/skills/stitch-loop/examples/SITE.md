...
> **AGENT INSTRUCTION:** Read this file before every iteration. It serves as the project's "Long-Term Memory." If `next-stitch-prompt.md` is empty, pick the highest priority item from Section 5 OR invent a new page that fits the project vision.
...
*If `next-stitch-prompt.md` is empty or completed, pick the next task from here.*
...
2. Always update `next-stitch-prompt.md` before completing.
