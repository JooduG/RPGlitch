...
const CHALK_HERESY_PATTERNS = [
  /#[0-9a-fA-F]{3,6}\b/, // Hex
  /rgba?\(\d+,\s*\d+,\s*\d+,?\s*[\d.]*\)/, // RGB/RGBA
  /hsla?\(\d+,\s*[\d.]+%,\s*[\d.]+%?,?\s*[\d.]*\)/, // HSL/HSLA
];
...
/**
 * 4. Architectural Heresy Detection (Rule 03 & 05)
 * Target: src/** (Svelte, JS, TS)
 */
function checkHeresy() {
  console.log("\n🧨 Heresy Detection (Rule 03):");
  ...
  function scanDir(dir) {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        if (BLACKLIST.includes(item) || item === ".agent") continue;
        scanDir(fullPath);
      } else if (stat.isFile() && /\.(js|ts|svelte|css)$/.test(item)) {
        // Skip tests and tokens.css
        if (item.endsWith(".test.js") || item.endsWith("tokens.css")) continue;
        ...
        lines.forEach((line, index) => {
          ...
          // Svelte Legacy Check
          if (item.endsWith(".svelte")) {
            SVELTE_LEGACY_PATTERNS.forEach((pattern) => {
              if (pattern.test(line)) {
                console.error(
                  `     ❌ ${path.relative(ROOT_DIR, fullPath)}:${index + 1} - Svelte 4 Heresy: ${pattern.source}`,
                );
                errorCount++;
              }
            });
          ...
          // Chalk Regime Check (Colors)
          if (/\.(svelte|css)$/.test(item)) {
            CHALK_HERESY_PATTERNS.forEach((pattern) => {
              // Ignore matches inside comments or if part of a variable name/comment line
              // This is a naive check but effective for raw hex
              if (pattern.test(line)) {
                // Heuristic: check if it's likely a color assignment or property
                if (line.includes(":") || line.includes("=") || line.includes("background")) {
                  console.error(
                    `     ❌ ${path.relative(ROOT_DIR, fullPath)}:${index + 1} - Chalk Heresy (Raw Color): ${line.match(pattern)}`,
                  );
                  errorCount++;
                }
              }
