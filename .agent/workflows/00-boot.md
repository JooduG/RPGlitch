...
3. **Validation**: Ensure neither the Mission Board nor log contain corrupted entries. Parse the "Current Goal" and identify the active baton. [[Invoke: project-manager]](../skills/project-manager/SKILL.md)

### Phase 2: Registry (Step 1.3: Prerequisites)

1. **Registry**: Identify any active track `[/]` or pending tasks `[ ]` in [log](../project-management/log.md). If no active track exists, wait for user input or initiate [/01-plan](./01-plan.md). [[Invoke: project-manager]](../skills/project-manager/SKILL.md)
...
- **🤖 AGENTS.md**: Step 1.5 (Intent Decoding - Bootstrapped)
- **📜 Rules**: Rule 01 (Foundation), Rule 05 (Intelligence)
- **🧠 Capabilities**: project-manager (Sync State)
