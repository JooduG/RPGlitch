...
- **The Master Baton [CRITICAL]**: `.agent/tasks/tracks.md` (The Mission Board - source of immediate truth and architectural history).
- **The Execution Baton [CRITICAL]**: `next-prompt.md` (The immediate active payload in the continuous loop).
- **Config**: `.agent/config.yaml`
- **Active Track**: `.agent/tasks/<slug>/<slug>.md` (The specific task plan if a track is active).
...
1. **Read The Master Baton**: You MUST read `.agent/tasks/tracks.md` immediately. This defines the exact current reality of the UI, active Svelte 5 paradigms, and which track is currently `[/]`.
2. **Read The Execution Baton**: You MUST read `next-prompt.md` to discover your immediate objective in the swarm loop.
3. **Load Active Track**: If a track is marked as `[/]` in `tracks.md`, read its corresponding plan file in `.agent/tasks/`.
...
3. **State Verification**:
    ...
    - If there is a mismatch (e.g., `next-prompt.md` is asking for a feature that `tracks.md` says is already completed), halt and trigger a **Status Report** to the Director.
...
- **Amnesia**: Attempting to write Svelte code or formulate a plan without reading `tracks.md` and `next-prompt.md` first.
