...
1. **Sync Configuration:**
   ...
   * *Goal:* Ensure all configs and ignore files are up to date.
2. **Lint & Fix:**
   * Command: `npm run lint:fix`
   * *Goal:* Auto-fix styling and JS issues.
3. **Test Suite:**
   * Command: `npm run test`
   * *Goal:* Verify logic integrity.
   * **CRITICAL:** If tests fail, **STOP IMMEDIATELY**. Report the failure and do not proceed to build.

## 2. The Build Process

We compile the Source (`apps/*/html/`) into the Artifact (`build/output/`).

1. **Execute Build:**
   * Command: `npm run build:apps`
   * *Goal:* Runs `esbuild` + `sass`, bundles JS (IIFE), and inlines assets.
2. **Verify Artifacts:**
...
1. **Analyze Changes:**
   ...
   * *Action:* Briefly summarize any *new* plugin imports or list structure changes since the last session.
2. **Instructions:**
