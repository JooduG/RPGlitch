...
const reversePluginMap = Object.fromEntries(
  Object.entries(pluginMap).map(([perchanceName, standardName]) => [
    standardName,
    perchanceName,
  ])
);
...
async function executeAiProcess(type, prompt, instructions) {
  ...
  if (areInstructionsMissing()) {
    console.error(
      "[ImageGlitch] AI instructions not loaded, attempting to reload..."
    );
    await loadAiInstructions();

    if (areInstructionsMissing()) {
      alert(
        "AI instructions failed to load. Please refresh the page and try again."
      );
      return;
    }
  }
...
async function handleSummonClick() {
  ...
  try {
    ...
    if (typeof image === "undefined") {
      console.error(
        "[ImageGlitch] text-to-image-plugin not loaded - image function unavailable"
      );
      alert(
...
function buildImageGenerationHtml() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (imageGenerator === "perchance") {
      outputHtml += `<div class="block quad-block" data-prompt="${encodeURIComponent(
        prompt
      )}">`;
      const resolutions = {
        "top-left": "768x768",
      ...
      const imgUrl = `${BASE_IMAGE_URL}${encodeURIComponent(
        prompt
      )}?${params.toString()}`;
      outputHtml += `<div class="block solo-block" data-seed="${blockSeed}" data-prompt="${encodeURIComponent(
        prompt
      )}"><img src="${imgUrl}" loading="lazy" alt="Generated image" crossorigin="anonymous"></div>`;
    }
  }
  return outputHtml;
}
...
async function waitForPlugins(
  ...
  const prefixedPlugins = requiredPlugins.map(
    (name) =>
      reversePluginMap[name] ||
      `plugin${name.charAt(0).toUpperCase()}${name.slice(1)}`
  );
  ...
  while (Date.now() - startTime < timeout) {
    ...
    if (allPluginsReady) {
      console.log(
        "[ImageGlitch] All plugins loaded successfully:",
        requiredPlugins
      );
      return true;
    }
...
async function loadAiInstructions() {
  ...
  if (TEST_MODE || typeof window.r === "undefined") {
    setAllFallbacks(
      "[ImageGlitch] Test mode or remember plugin unavailable, using fallback instructions"
    );
    return;
  }
  ...
  try {
    ...
    const [scribeInstruction, chaosInstruction, transfigureInstruction] =
      await Promise.all([
    ...
    if (scribeInstruction) {
      console.log(
        "[ImageGlitch] Loaded aiScribeInstruction from remember plugin"
      );
    } else {
      console.warn(
        "[ImageGlitch] aiScribeInstruction not found in remember plugin, using fallback"
      );
    }
    ...
    if (chaosInstruction) {
      console.log(
        "[ImageGlitch] Loaded aiChaosInstruction from remember plugin"
      );
    } else {
      console.warn(
        "[ImageGlitch] aiChaosInstruction not found in remember plugin, using fallback"
      );
    }
    ...
    if (transfigureInstruction) {
      console.log(
        "[ImageGlitch] Loaded aiTransfigureInstruction from remember plugin"
      );
    } else {
      console.warn(
        "[ImageGlitch] aiTransfigureInstruction not found in remember plugin, using fallback"
      );
    }
  } catch (error) {
    setAllFallbacks(
      "[ImageGlitch] Failed to load AI instructions from remember plugin, using fallbacks:",
      error
    );
  }
}
  ...
  function updatePromptWarning() {
    if (warningEl) {
      warningEl.style.display =
        promptInput.value.length > MAX_PROMPT_LENGTH ? "block" : "none";
    }
  }
...
function addImageOverlays() {
  document.querySelectorAll(".solo-block").forEach((block) => {
    ...
    const sanitizedPrompt = window.DOMPurify
...
function downloadImage(container) {
  ...
  if (imgEl) {
      ...
      fetch(imgEl.src)
        ...
        .catch((err) => {
          ...
          try {
            ...
            alert(
              "Failed to download the image. Please try right-clicking and using 'Save Image As' instead."
            );
          }
        });
    } catch (e) {
    ...
    try {
      ...
      alert(
        "Failed to download the image. This may be due to cross-origin restrictions."
      );
    }
  }
}
...
function rerollImage(container, resolution) {
  ...
  const newSeed =
    window.crypto.getRandomValues(new Uint32Array(1))[0] % 10000000;
  ...
  if (container.classList.contains("solo-block")) {
    ...
    if (imgEl) {
      const newUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(
        prompt
      )}?width=1280&height=1280&seed=${newSeed}&model=flux&private=true&nologo=true`;
      imgEl.src = newUrl;
