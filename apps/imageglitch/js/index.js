...
function buildImageGenerationHtml() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (Math.random() < 0.1) {
      ...
      outputHtml += `<div class="block solo-block" data-seed="${blockSeed}" data-prompt="${encodeURIComponent(prompt)}"><img src="${imgUrl}" loading="lazy" alt="Generated image"></div>`;
    } else {
...
function addImageOverlays() {
  ...
  document.querySelectorAll('.quad-cell').forEach(cell => {
    ...
    const prompt = decodeURIComponent(quadBlock.dataset.prompt);
...
function rerollImage(container, resolution) {
  const prompt = decodeURIComponent(container.dataset.prompt);
