...
db.version(2).stores({
  entities:
    "++id, name, type, updated, avatar, persona, scenario, tags, createdAt, updatedAt, isSelected, [type+isCustom]",
  ...
  .upgrade(async (trans) => {
    try {
      ...
      try {
        ...
        existingSettings.debugMode = /^(1|true)$/i.test(
          String(storedDebug).trim()
        );
      } catch {
      ...
      try {
        const storedSelection = localStorage.getItem(
          "rpglitch-storyboard-selection"
        );
        if (storedSelection) {
          existingSettings.storyboardSelection = JSON.parse(storedSelection);
        } else {
          existingSettings.storyboardSelection = {
            ai: null,
            user: null,
            world: null,
          };
      ...
      console.log(
        "[RPGlitch DB Migration] Migration v3 completed successfully."
      );
    } catch (err) {
    ...
    try {
      ...
      if (storiesJSON) {
        ...
        if (Array.isArray(stories) && stories.length > 0) {
          ...
          console.log(
            `[Migration v4] Migrated ${storyEntities.length} stories from localStorage to IndexedDB`
          );
        }
      }
  ...
  .upgrade(async (trans) => {
    ...
    try {
      ...
      if (!existing) {
        ...
        console.log(
          "[RPGlitch DB Migration] Created default settings singleton."
        );
      }
    } catch (err) {
...
db.version(7)
  ...
  .upgrade(async (trans) => {
    try {
      console.log(
        "[RPGlitch DB Migration] Starting migration v7: Add signatureColour..."
      );
      const entities = trans.table("entities");
      ...
      console.log(
        "[RPGlitch DB Migration] Migration v7 completed successfully."
      );
    } catch (err) {
...
export async function init() {
  try {
    ...
    if (
      ...
      console.warn(
        "[RPGlitch DB] Schema corruption detected. Deleting and recreating database..."
      );
      ...
      try {
        ...
        newDb.version(5).stores({
          entities:
            "++id, name, type, updated, avatar, persona, scenario, tags, createdAt, updatedAt, isSelected, [type+isCustom]",
          threads:
            "++id, characterId, title, settingsSnapshot, createdAt, updatedAt",
          messages: "++id, threadId, role, text, seed, meta, createdAt",
          settings: "id",
        });
        ...
        await newDb.open();
        console.log(
          "[RPGlitch DB] Fresh database created and opened successfully."
        );
        ...
        console.error(
          "[RPGlitch DB] Failed to recover from corruption:",
          deleteErr
        );
        throw deleteErr;
      }
