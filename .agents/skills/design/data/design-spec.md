...
A format specification for describing a visual identity to coding agents. DESIGN.md gives agents a persistent, structured understanding of a design system. From [Github DESIGN.md](https://github.com/google-labs-code/design.md)
...
The `<scale-level>` placeholder represents a named level in a sizing or spacing scale. Common level names include `tiny`, `small`, `medium`, `large`, `giant`, and `full`. Any descriptive string key is valid.
...
**Typography:** `headline-display`, `headline-large`, `headline-medium`, `body-large`, `body-medium`, `body-small`, `label-large`, `label-medium`, `label-small`

**Rounded:** `none`, `small`, `medium`, `large`, `giant`, `full`
