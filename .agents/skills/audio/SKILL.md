...
### 1. The Voice Engine (Kokoro-82M Neural TTS & `speech.js`)

The core audio logic resides in `src/media/audio.svelte.js` (Audio effects and master state) complemented by `src/media/speech.js` (pure helpers for sentence segmentation, dialogue speaker attribution, cadence rates, and the Kokoro voice catalog). It exports a singleton `Audio` instance via `@media`.
...
- **Voice Catalog**: 28 total voices (13 Male: `am_*`, `bm_*`; 15 Female: `af_*`, `bf_*` with `af_heart` default) exported as `KOKORO_VOICES` from `src/media/speech.js`.
...
- **Pure Helpers (`src/media/speech.js`)**: Handles quote-aware sentence splitting (`split_speech_sentences`), dialogue-speaker attribution, typographic emphasis parsing, and speaking rate calculation.
