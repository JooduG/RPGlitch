...
export async function bootstrapPrompt(): Promise<string> {
  ...
  return `You are the swarm planner for ${repoInfo.fullName}. Your job is to fetch open GitHub issues, analyze them, and dispatch parallel coding agents.
...
After writing both \`.agents/archive/swarm/\` files, run the dispatcher:

\`\`\`bash
npm run swarm:dispatch
\`\`\`

This dispatches parallel Jules sessions for each task and logs session IDs to \`.agents/archive/swarm/{date}/sessions.json\`.
