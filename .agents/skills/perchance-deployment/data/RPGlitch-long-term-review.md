...
| 3.3 | Genesis & promotion engine                                         | `spawn_npc` / Director `genesis` + `promotions` persist to Dexie                                                                                    | `kernel.spawn_npc(bridge, draft)` + Director `"genesis"` payload (`_apply_genesis` mints new entities with exact signature color, stage entry, background portrait, and Dexie persistence); `_apply_promotions` clamps tier to 2/3 and persists | `normalizer.test.js`, `kernel.test.js`, `director.test.js` (genesis & promotions normalization)     |         |
...
4. **NPC genesis supports Director requests** — the Director LLM can request brand-new cast members via `"genesis"` (name, description, registry `signature_color`) when no `<WORLD_CAST>` entity fits; otherwise the Entity Convergence Law mandates reusing existing cast members.
...
6. ✨ **NPC Genesis & Creation Lifecycle (T6, T16)**: Introduce a scenario requiring an entirely new character where no existing `<WORLD_CAST>` member applies (e.g. entering an unfamiliar black market and asking for an underground courier).
   - _Gate_: The Director emits a valid `"genesis"` payload in JSON (name + one-line description + `signature_color` strictly from `<AVAILABLE_SIGNATURE_COLORS>`). The Kernel creates the entity with `role_tier: 1` (or 2), persists it to Dexie, puts the new NPC on-stage (`in_scene_npc_ids`), and dispatches a background portrait request without adding turn latency.
7. 🧩 **Entity Convergence Law (T7, T18)**: Reference a doctor in a clinic scene where Dr. Elias Tariq is in the world cast.
   - _Gate_: The Director/AI reuses the existing cast member — zero invented duplicate doctors; `<WORLD_CAST>` signatures remain compact and the active trio is never listed as a summonable stranger.
...
- [ ] **Genesis & Promotions**: Director `genesis` payload creates new NPC with registry signature color, puts them on-stage, fires background portrait, and persists to Dexie; `promotions` clamp to tier 2/3.
